...
const SearchingHeader = ({ onCancel }) => {
  ...
  const searching = useCallback(
    () => {
      if(JSON.stringify(searchValue) !== JSON.stringify({search: ""})){
        console.log("Searching...", searchValue);
      }
      s.addEventListener("submit", onSubmit)
      navigation.addListener("state", onSubmit);
      return()=> {
        navigation.removeListener("state", onSubmit);
      }
    },
    [searchValue],
  )
